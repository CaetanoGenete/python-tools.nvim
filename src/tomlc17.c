...
static int parse_key(parser_t *pp, token_t tok, keypart_t *ret_keypart)
{
	...
	if (parse_norm(pp, tok, &kpspan[n])) {
		return RETERROR(pp->ebuf, tok.lineno, "unable to normalize string; probably a unicode issue");
	}
	n++;
...
static toml_datum_t *descend_keypart(
  parser_t *pp, int lineno, toml_datum_t *toptab, keypart_t *keypart, bool stdtabexpr)
{
	...
	for (int i = 0; i < keypart->nspan; i++) {
		...
		if (value->type == TOML_ARRAY) {
			...
			if (value->u.arr.size <= 0) {
				RETERROR(pp->ebuf, lineno, "array %s has no elements", keypart->span[i].ptr);
				return NULL;
			}
			...
			// It must be a table!
			if (value->type != TOML_TABLE) {
				RETERROR(pp->ebuf, lineno, "array %s must be array of tables", keypart->span[i].ptr);
				return NULL;
			}
			tab = value; // descend
			continue;
		...
		// key not found
		RETERROR(pp->ebuf, lineno, "cannot locate table at key %s", keypart->span[i].ptr);
		return NULL;
	}
	...
	*ret_datum = mkdatum(TOML_ARRAY);
	...
	for (;;) {
		...
		if (tok.toktyp == TOK_COMMA) {
			...
			return RETERROR(pp->ebuf, tok.lineno, "syntax error while parsing array: unexpected comma");
		}

		// Not a comma, but need a comma: error!
		if (need_comma) {
			return RETERROR(pp->ebuf, tok.lineno, "syntax error while parsing array: missing comma");
		}
...
static int parse_array_table_expr(parser_t *pp, token_t tok)
{
	...
	for (int i = 0; i < keypart.nspan; i++) {
		...
		if (value->type == TOML_ARRAY) {
			...
			if (value->u.arr.size <= 0) {
				return RETERROR(pp->ebuf, keylineno, "array %s has no elements", curkey.ptr);
			}
			value = &value->u.arr.elem[value->u.arr.size - 1];
			if (value->type != TOML_TABLE) {
				return RETERROR(pp->ebuf, keylineno, "array %s must be array of tables", curkey.ptr);
			}
			tab = value;
			continue;
		}
		...
		// keypart not found
		return RETERROR(pp->ebuf, keylineno, "cannot locate table at key %s", curkey.ptr);
	}
...
static int parse_keyvalue_expr(parser_t *pp, token_t tok)
{
	...
	for (int i = 0; i < keypart.nspan - 1; i++) {
		...
		return RETERROR(pp->ebuf, keylineno, "cannot locate table at '%s'", keypart.span[i].ptr);
	}
...
static int parse_norm(parser_t *pp, token_t tok, span_t *ret_span)
{
	...
	while (*p) {
		...
		case 'U': {
			...
			if (n < 0) {
				return RETERROR(pp->ebuf, tok.lineno, "error converting UCS %s to UTF8", buf);
			}
			dst += n;
			p += 2 + sz;
		} break;
	...
	while (1) {
		if (S_MATCH3('"')) {
			if (S_MATCH4('"')) {
				...
				if (S_MATCH6('"')) {
					return RETERROR(sp->ebuf, sp->lineno, "detected sequences of 3 or more double quotes");
				}
				else {
					; // no problem
				}
		...
		if (ch == 'u' || ch == 'U') {
			...
			for (int i = 0; i < top; i++) {
				if (!is_hex_char(S_GET())) {
					return RETERROR(sp->ebuf, sp->lineno, "expect %d hex digits after \\%c", top, ch);
				}
			}
			continue;
		}
	...
	while (1) {
		if (S_MATCH3('\'')) {
			if (S_MATCH4('\'')) {
				...
				if (S_MATCH6('\'')) {
					return RETERROR(sp->ebuf, sp->lineno, "sequences of 3 or more single quotes");
				}
				else {
					; // no problem
				}
...
static int scan_timestamp(scanner_t *sp, token_t *tok)
{
	...
	toktyp = TOK_DATE;
	p += n;
	if (!((p[0] == 'T' || p[0] == ' ' || p[0] == 't') && isdigit(p[1]) && isdigit(p[2])
	      && p[3] == ':')) {
		goto done; // date only
	}
...
{
	return &p[4] < endp && isdigit(p[0]) && isdigit(p[1]) && isdigit(p[2]) && isdigit(p[3])
	       && p[4] == '-';
}
